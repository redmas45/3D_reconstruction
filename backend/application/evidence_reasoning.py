...
REASONING_PROMPT_VERSION = "visible-story-planner-v4-scored-candidates"
...
def _select_decisions(
    ...
    if cached is not None:
        safe_cached = validate_gap_decisions(
            enforce_scored_gap_decisions(cached[0], artifacts["hypotheses"]),
            artifacts["ledger"]["evidence_digest"], artifacts["clues"], artifacts["hypotheses"],
        )
        return "azure_cache", safe_cached, cached[1]
    if settings is None:
    ...
    try:
        ...
        validated = validate_gap_decisions(
            enforce_scored_gap_decisions(validated, artifacts["hypotheses"]),
            artifacts["ledger"]["evidence_digest"],
            artifacts["clues"], artifacts["hypotheses"],
        )
