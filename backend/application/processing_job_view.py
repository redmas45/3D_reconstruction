...
# The scene manifest carries per-entity geometry (waypoints, proxies) for a crowded
# frame, so it is allowed a larger ceiling than the text-only reasoning summary.
MAXIMUM_SCENE_MANIFEST_BYTES = 6_000_000
...
def build_public_job_record(record: ProcessingJob) -> dict[str, object]:
    ...
    return {
        ...
        "scene": _scene_snapshot(record.output_dir),
    }


def _scene_snapshot(output_directory: Path) -> dict | None:
    work_directory = output_directory / "_work"
    if not work_directory.is_dir():
        return None
    try:
        manifest_paths = list(work_directory.glob(f"*/{SCENE_MANIFEST_FILENAME}"))
    except OSError:
        return None
    if len(manifest_paths) != 1 or not _is_bounded_file(
        manifest_paths[0], MAXIMUM_SCENE_MANIFEST_BYTES,
    ):
        return None
    payload = read_json_file(manifest_paths[0])
    return payload if _valid_scene_manifest(payload) else None


def _valid_scene_manifest(payload: object) -> bool:
    return (
        isinstance(payload, dict)
        and payload.get("schema_version") == SCENE_MANIFEST_SCHEMA_VERSION
        and payload.get("renderer") == "threejs"
        and isinstance(payload.get("source"), dict)
        and isinstance(payload.get("gaps"), list)
        and len(payload["gaps"]) <= MAXIMUM_PUBLIC_REASONING_ITEMS
    )
...
def _is_bounded_file(path: Path, maximum_bytes: int = MAXIMUM_PUBLIC_REASONING_BYTES) -> bool:
    try:
