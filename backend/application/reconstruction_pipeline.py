"""Coordinates evidence analysis and browser scene-manifest generation."""
...
from application.browser_playback import BrowserPlaybackError, write_browser_playback_video
from application.plate_evidence import PlateEvidenceError, resolve_clean_plate
from application.scene_manifest import build_and_write_scene_manifest
from application.scene_planning import prepare_scene_assets
...
DEFAULT_CONFIG_PATH = BACKEND_ROOT / "config" / "reconstruction_config.json"
DEFAULT_MODEL_PATH = "backend/models/yolo26m.pt"
DEFAULT_POSE_MODEL_PATH = "backend/models/yolo26n-pose.pt"
...
# The browser is the active renderer. The backend's job ends at a validated scene
# manifest, which the Three.js client draws and animates; no server-side 3D render
# runs. The legacy value "blender" is honoured only when a run explicitly asks for it.
BROWSER_SCENE_RENDERER = "threejs"
SCENE_MANIFEST_PROGRESS = 0.9
# A browser gap base must be visibly sharp. Once this much of a clean plate is
# unresolved, its blur-fill becomes a human-shaped ghost; the playback writer then
# uses a moving transition between fully observed boundary frames instead.
MAX_BROWSER_PLATE_UNRESOLVED_FRACTION = 0.02
...
# Retained only for the explicitly disabled legacy renderer functions below. The
# browser path never imports or evaluates Blender.
DEFAULT_RENDER_STALL_TIMEOUT_SECONDS = 7200
...
def _load_detections(
    ...
    detections = detect_scene_objects(
        ...
        model_name=yolo_config.get("model", DEFAULT_MODEL_PATH),
        ...
        pose_model_name=(
            yolo_config.get("pose_model", DEFAULT_POSE_MODEL_PATH)
...
def _detection_cache_contract(
    ...
    return {
        ...
        "model": str(yolo_config.get("model", DEFAULT_MODEL_PATH)),
        "class_ids": yolo_class_ids(config),
        ...
        "pose_model": str(yolo_config.get("pose_model", DEFAULT_POSE_MODEL_PATH)),
...
def _render_visible_segment(
    ...
    if not bool(visual_config.get("production_overlays", False)):
        # Evidence overlays belong in the dashboard, never burned into the judge-facing
        # playback. The source timeline must remain visually identical outside gaps.
        write_video_range(
            video_path,
            int(segment["start"]),
            int(segment["end"]),
...
def _render_blender_hidden_segment(
    ...
    from application.blender_pipeline import render_blender_gap
    ...
    output_path = render_blender_gap(
        ROOT,
        prepared.scene_plan_paths[gap_index],
        gap_directory,
        context.reuse_work,
    ...
    plan = json.loads(
        prepared.scene_plan_paths[gap_index].read_text(encoding="utf-8"),
    )
...
def _render_blender_gaps_with_budget(
    ...
    plans = _read_blender_plans(context.prepared.scene_plan_paths)
    ...
    elapsed_seconds = _representative_elapsed_seconds(
        context.prepared.scene_plan_paths[representative_index],
...
def _actor_render_blocked(plans: list[dict]) -> str | None:
    ...
    """
    from application.actor_render_job import actor_path_is_supported
...
def _render_actor_gaps(
    ...
    prepared = context.prepared
    plans = _read_blender_plans(prepared.scene_plan_paths)
...
def _validate_actor_gap_videos(
    ...
    from application.actor_render_job import ActorJobError
...
def _build_actor_job(
    ...
    from application.actor_render_job import ActorJobError, ActorRenderJob
    ...
    return ActorRenderJob(
        ...
        library_directory=BACKEND_ROOT / "assets" / "actors",
        cancellation_check=context.cancellation_check,
...
def _require_representative_preview_approval(
    ...
    if not renderer.get("interactive_preview_approval", False):
        return
    plan_path = context.prepared.scene_plan_paths[representative_index]
...
def _evaluate(
    ...
    return evaluate_reconstructions(
        items,
        str(video_path),
        yolo_config.get("model", DEFAULT_MODEL_PATH),
...
def _prepare_reconstruction(
    ...
    _validate_runtime_dependencies(_active_renderer(config) == "blender")
    ...
    scene_report, scene_plan_paths = _prepare_scene_and_plans(
    ...
    return PreparedReconstruction(
        info, selection, segment_paths, scene_report, work_dir, scene_plan_paths,
...
def _prepare_scene_and_plans(
    ...
    scene_assets = prepare_scene_assets(
        video_path,
        scene_report,
    ...
    scene_report = scene_assets.scene_report
    write_json(work_dir / "scene_report.json", scene_report)
    return scene_report, scene_assets.plan_paths
...
def _validate_runtime_dependencies(require_blender: bool = False) -> None:
    find_media_tool("ffmpeg")
    find_media_tool("ffprobe")
    # The browser-first path never invokes a server-side 3D renderer, so it must not
    # demand a Blender install. Only the explicitly-opted-in legacy path checks for it.
    if require_blender:
        find_blender_executable()


def find_blender_executable():
    """Resolve the archived renderer only when an old job explicitly asks for it."""
    from infrastructure.blender_runner import find_blender_executable as resolve_blender

    return resolve_blender()


def _active_renderer(config: dict) -> str:
    return str(config.get("renderer", {}).get("active_renderer", BROWSER_SCENE_RENDERER))


def _finalize_browser_scene(
    video_path: Path,
    options: PipelineOptions,
    prepared: PreparedReconstruction,
    progress_callback: ProgressCallback | None,
) -> Path:
    """Build the manifest and a frame-complete playback base for the browser.

    The returned MP4 is deliberately not a second, unrelated video. It contains every
    source frame in its original timeline order, except selected gaps, where frames are
    replaced by a visible-evidence background plate. The browser places the generated
    Three.js actors over those exact intervals, so judges see one normal video instead
    of a separate 3D demo beside it.
    """
    _report(
        progress_callback, "building_scene", SCENE_MANIFEST_PROGRESS,
        "Assembling the validated Three.js scene manifest",
    )
    build_and_write_scene_manifest(
        prepared.video_info,
        prepared.gap_selection,
        prepared.scene_report,
        prepared.scene_plan_paths,
    ...
    _report(
        progress_callback, "stitching", STITCHING_PROGRESS,
        "Preparing the original timeline with safe replacement intervals",
    )
    plans = _read_blender_plans(prepared.scene_plan_paths)
    plates = _browser_plate_provider(
        video_path, options, prepared, plans, progress_callback,
    )
    playback_directory = prepared.work_dir / "playback"
    video_only_path = playback_directory / "video_only.mp4"
    try:
        write_browser_playback_video(
            video_path,
            video_only_path,
            [tuple(item) for item in prepared.gap_selection["hidden_ranges"]],
            plates,
            options.cancellation_check,
        )
    except BrowserPlaybackError:
        LOGGER.exception("Could not build the evidence-safe browser playback base")
        raise
    final_output = options.output_dir / f"{video_path.stem}_reconstructed.mp4"
    encode_with_source_audio(
        video_only_path, video_path, final_output, options.cancellation_check,
    )
    validate_video_contract(final_output, VideoContract(
        prepared.video_info["width"],
        prepared.video_info["height"],
    ...
    write_json_file(
        prepared.work_dir / "diagnostic_report.json",
        {"mode": "browser_timeline_overlay", "status": "not_applicable"},
    )
    _report(
        progress_callback, "completed", COMPLETED_PROGRESS,
        "Original video timeline ready with Three.js gap overlays",
    )
    return final_output


def _browser_plate_provider(
    video_path: Path,
    options: PipelineOptions,
    prepared: PreparedReconstruction,
    plans: list[dict],
    progress_callback: ProgressCallback | None,
) -> Callable[[int], numpy.ndarray | None] | None:
    """Recover one visible-only background plate per shot for hidden intervals."""
    if not plans:
        return None
    context = TimelineRenderContext(
        video_path,
    ...
    try:
        plate_for_gap = _resolve_pipeline_plates(context, plans, progress_callback)
        return _quality_gated_browser_plates(prepared, plans, plate_for_gap, progress_callback)
    except PlateEvidenceError as error:
        LOGGER.warning("Browser playback will use boundary fallbacks: %s", error)
        _report(
            progress_callback,
            "stitching",
            STITCHING_PROGRESS,
            "Using visible boundary frames because a clean plate was unavailable",
        )
        return None


def _quality_gated_browser_plates(
    prepared: PreparedReconstruction,
    plans: list[dict],
    plate_for_gap: Callable[[int], numpy.ndarray],
    progress_callback: ProgressCallback | None,
) -> Callable[[int], numpy.ndarray | None]:
    """Reject ghost-prone plates while preserving good plates from other shots."""
    shots = _pipeline_shots(prepared)
    gap_shots = {
        int(plan["gap_index"]): _shot_for_plan(shots, plan) for plan in plans
    }
    usable_shots: dict[int, bool] = {}
    for shot_index in sorted(set(gap_shots.values())):
        report_path = _plate_directory(
            prepared.work_dir, shot_index, len(shots),
        ) / "clean_plate_report.json"
        report = read_json_file(report_path)
        unresolved_fraction = (
            float(report.get("unresolved_pixel_fraction", 1.0))
            if isinstance(report, dict)
            else 1.0
        )
        stable = bool(report.get("stable", False)) if isinstance(report, dict) else False
        usable = stable and unresolved_fraction <= MAX_BROWSER_PLATE_UNRESOLVED_FRACTION
        usable_shots[shot_index] = usable
        if not usable:
            _report(
                progress_callback,
                "stitching",
                STITCHING_PROGRESS,
                f"Using an evidence-safe boundary transition for take {shot_index + 1}; "
                f"the recovered plate left {unresolved_fraction * 100:.1f}% unresolved",
            )

    def safe_plate_for_gap(gap_index: int) -> numpy.ndarray | None:
        shot_index = gap_shots.get(gap_index)
        if shot_index is None or not usable_shots.get(shot_index, False):
            return None
        return plate_for_gap(gap_index)

    return safe_plate_for_gap
...
def process_video(
    ...
    if _active_renderer(options.config_data) != "blender":
        return _finalize_browser_scene(video_path, options, prepared, progress_callback)
