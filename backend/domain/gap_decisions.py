...
from domain.hypothesis_scoring import choose_safe_hypothesis
...
SUPPORTED_SELECTION_SOURCES = frozenset({"azure_reasoner", "deterministic_ranker", "safety_gate"})
...
def enforce_scored_gap_decisions(gap_decisions: dict, hypothesis_library: dict) -> dict:
    """Apply deterministic safety bounds after any model decision.

    The language model may rank and explain bounded candidates, but it cannot select
    a candidate that violates lifecycle constraints or is materially worse than the
    measured best candidate.
    """
    updated = copy.deepcopy(gap_decisions)
    hypotheses = _hypotheses_by_gap_and_entity(hypothesis_library)
    for gap_decision in updated.get("decisions", []):
        gap_index = int(gap_decision["gap_index"])
        for entity_decision in gap_decision.get("entities", []):
            entity_id = str(entity_decision["entity_id"])
            candidates = list(hypotheses[gap_index][entity_id].values())
            selected, overridden = choose_safe_hypothesis(
                candidates, str(entity_decision["selected_hypothesis_id"]),
            )
            if overridden:
                _apply_safety_override(entity_decision, selected)
            else:
                entity_decision.setdefault("selection_source", "azure_reasoner")
    return updated
...
def _entity_schema(text: dict) -> dict:
    return {
        ...
        "properties": {
            ...
            "confidence": {"type": "number", "minimum": 0, "maximum": 1},
            "selection_source": {
                "type": "string", "enum": sorted(SUPPORTED_SELECTION_SOURCES),
            },
        },
    }
...
def _fallback_entity_decision(entity: dict) -> dict:
    ...
    return {
        ...
        "confidence": float(selected.get("selection_score", selected["prior"])),
        "selection_source": "deterministic_ranker",
    }
...
def _fallback_hypothesis(hypotheses: list[dict]) -> dict:
    ...
    return max(
        candidates,
        key=lambda item: float(item.get("selection_score", item.get("prior", 0.0))),
    )
...
def _validate_entity_decision(value: object, hypotheses: dict[str, set[str]]) -> dict:
    ...
    selection_source = value.get("selection_source", "azure_reasoner")
    if selection_source not in SUPPORTED_SELECTION_SOURCES:
        raise GapDecisionValidationError("Entity selection source is invalid")
    return {
        "entity_id": entity_id,
...
def _apply_safety_override(entity_decision: dict, selected: dict) -> None:
    original_id = str(entity_decision["selected_hypothesis_id"])
    selected_id = str(selected["id"])
    rejected = [
        item for item in entity_decision.get("rejected_hypotheses", [])
        if isinstance(item, dict) and item.get("id") != selected_id
    ]
    if original_id != selected_id and not any(item.get("id") == original_id for item in rejected):
        rejected.append({
            "id": original_id,
            "reason": "Safety gate rejected the model choice because a measured candidate was materially safer.",
        })
    entity_decision["selected_hypothesis_id"] = selected_id
    entity_decision["rejected_hypotheses"] = rejected[:MAXIMUM_LIST_ITEMS]
    entity_decision["confidence"] = float(selected.get("selection_score", 0.0))
    entity_decision["decision_summary"] = (
        f"Safety gate selected {selected.get('type', 'bounded motion')} from visible measurements."
    )
    entity_decision["selection_source"] = "safety_gate"
