...
# Detector samples are intentionally sparse (the default tracker samples every few
# frames).  A short boundary window can therefore contain only one sample, which
# collapses a moving person to a zero-velocity hold.  The adaptive window below stays
# inside visible evidence while covering at least two normal detector samples.
MINIMUM_BOUNDARY_WINDOW_SECONDS = 0.75
MAXIMUM_BOUNDARY_WINDOW_SECONDS = 2.0
MAXIMUM_BOUNDARY_SAMPLES = 3
...
def _boundary_detections(
    ...
    window_frames = _boundary_window_frames(detections, boundary_frame, side, fps)
    if side == "before":
        eligible = [
            item for item in detections
            if boundary_frame - window_frames <= item["frame"] < boundary_frame
        ]
        return eligible[-MAXIMUM_BOUNDARY_SAMPLES:]
    eligible = [
        item for item in detections
...
def _boundary_window_frames(
    detections: list[dict],
    boundary_frame: int,
    side: str,
    fps: float,
) -> int:
    minimum_frames = max(2, math.ceil(MINIMUM_BOUNDARY_WINDOW_SECONDS * fps))
    maximum_frames = max(minimum_frames, math.ceil(MAXIMUM_BOUNDARY_WINDOW_SECONDS * fps))
    nearby = (
        [item for item in detections if int(item["frame"]) < boundary_frame]
        if side == "before"
        else [item for item in detections if int(item["frame"]) > boundary_frame]
    )
    nearby = nearby[-6:] if side == "before" else nearby[:6]
    intervals = [
        int(second["frame"]) - int(first["frame"])
        for first, second in zip(nearby, nearby[1:])
        if int(second["frame"]) > int(first["frame"])
    ]
    cadence_frames = int(round(median(intervals))) if intervals else minimum_frames
    return min(maximum_frames, max(minimum_frames, 2 * cadence_frames))
...
def _boundary_evidence(
    ...
    post_gap_world = _post_gap_world(after, frame_size, camera_contract) if before else None
    residual = _distance(world_waypoints[-1]["world"], post_gap_world) if post_gap_world else None
    return {
        ...
        "pre_gap_world": list(world_waypoints[0]["world"]) if world_waypoints else None,
        "predicted_end_world": list(world_waypoints[-1]["world"]) if world_waypoints else None,
        "post_gap_world": post_gap_world,
    }
...
def _post_gap_residual(
    ...
    post_world = _post_gap_world(after, frame_size, camera_contract)
    return _distance(world_waypoints[-1]["world"], post_world) if post_world else None


def _post_gap_world(
    after: list[dict], frame_size: tuple[int, int], camera_contract: dict,
) -> list[float] | None:
    if not after:
        return None
    post_point = _ground_point(after[0])
    return image_point_to_world(
...
def _image_velocity(detections: list[dict]) -> tuple[float, float]:
    ...
    for first, last in zip(detections, detections[1:]):
    ...
    return (
        float(median(item[0] for item in velocities)),
        float(median(item[1] for item in velocities)),
    )
