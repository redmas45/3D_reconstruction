...
Run it with:
    ...
    python -m uvicorn interfaces.api.app:app --app-dir backend --port 8000 --reload
...
UPLOAD_ROOT = BACKEND_ROOT / "runtime" / "data" / "uploads"
OUTPUT_ROOT = BACKEND_ROOT / "runtime" / "outputs"
...
# The supported browser is same-origin through app.py. These localhost origins remain
# available for API-only development tools without making the API public.
DEVELOPMENT_ORIGINS = (
...
def health() -> dict:
    ...
    return {
        ...
        "blender": {
            "available": False,
            "detail": "Inactive: the supported renderer is Three.js in the browser",
        },
