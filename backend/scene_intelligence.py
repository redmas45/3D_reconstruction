...
AMBIGUOUS_REIDENTIFICATION_MARGIN = 0.06
...
def _reidentification_score(before: dict, after: dict) -> dict:
    ...
    motion = _motion_continuity(before, after)
    score = 0.0 if appearance_conflict else (
        (0.45 * appearance) + (0.25 * spatial) + (0.20 * motion) + (0.10 * area_ratio)
    )
    return {
        "score": round(score, 4),
...
def _motion_continuity(before: dict, after: dict) -> float:
    before_velocity = _center_velocity(before, "end")
    after_velocity = _center_velocity(after, "start")
    before_speed = math.hypot(*before_velocity)
    after_speed = math.hypot(*after_velocity)
    if before_speed < 1e-6 or after_speed < 1e-6:
        return 0.50
    angle = abs(
        (math.degrees(math.atan2(after_velocity[1], after_velocity[0]))
         - math.degrees(math.atan2(before_velocity[1], before_velocity[0])) + 180.0) % 360.0 - 180.0
    )
    direction_fit = max(0.0, 1.0 - angle / 120.0)
    speed_ratio = min(before_speed, after_speed) / max(before_speed, after_speed)
    return 0.70 * direction_fit + 0.30 * speed_ratio
...
def _select_one_to_one_matches(candidates: list[tuple[float, int, int, dict]]) -> list[tuple[int, int, dict]]:
    ...
    for score, before_index, after_index, evidence in candidates:
        if score < REIDENTIFICATION_THRESHOLD:
            break
        if _is_ambiguous_candidate(candidates, before_index, after_index, score):
            continue
...
def _is_ambiguous_candidate(
    ...
    alternatives = [
        other_score for other_score, other_before, other_after, _ in candidates
        if (other_before == before_index and other_after != after_index)
        or (other_after == after_index and other_before != before_index)
    ]
    return bool(alternatives) and max(alternatives) >= score - AMBIGUOUS_REIDENTIFICATION_MARGIN
