...
class EvidenceTimelineTests(unittest.TestCase):
    ...
    def test_browser_first_path_checks_media_tools_but_not_blender(self) -> None:
        # The active renderer is the browser, so requiring a Blender install here would
        # block a run that never invokes one.
    ...
    def test_legacy_path_still_checks_blender_when_explicitly_requested(self) -> None:
        with patch("application.reconstruction_pipeline.find_media_tool") as media_tool:
            with patch("application.reconstruction_pipeline.find_blender_executable") as blender_tool:
                _validate_runtime_dependencies(require_blender=True)
                self.assertEqual([call("ffmpeg"), call("ffprobe")], media_tool.call_args_list)
