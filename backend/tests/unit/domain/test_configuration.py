...
class ConfigurationValidationTests(unittest.TestCase):
    def setUp(self) -> None:
        self.configuration = load_validated_configuration(PROJECT_ROOT / "backend" / "config" / "reconstruction_config.json")

    def test_project_configuration_is_valid(self) -> None:
        validate_configuration(self.configuration)
        self.assertEqual(PROJECT_ROOT / "backend" / "config" / "reconstruction_config.json", DEFAULT_CONFIG_PATH)
