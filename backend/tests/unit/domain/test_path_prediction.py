...
class PathPredictionTests(unittest.TestCase):
    ...
    def test_sparse_detector_samples_still_recover_boundary_motion(self) -> None:
        camera = build_camera_contract(_scene_report())
        track = {
            "id": "person_sparse",
            "class_name": "person",
            "continuity_confidence": 0.9,
            "detections": [
                {"frame": 96, "bbox": [90, 100, 110, 200], "confidence": 0.9},
                {"frame": 112, "bbox": [122, 100, 142, 200], "confidence": 0.9},
                {"frame": 292, "bbox": [250, 100, 270, 200], "confidence": 0.9},
                {"frame": 308, "bbox": [282, 100, 302, 200], "confidence": 0.9},
            ],
        }

        prediction = build_entity_prediction(track, (115, 291), 29.97, (640, 480), camera)

        self.assertIsNotNone(prediction)
        self.assertEqual("continuous", prediction["lifecycle"])
        self.assertGreater(prediction["speed_meters_per_second"], 0.0)
        self.assertNotEqual(
            prediction["path_prediction"]["waypoints"][0]["world"],
            prediction["path_prediction"]["waypoints"][-1]["world"],
        )

    def test_single_boundary_sample_remains_conservative(self) -> None:
        camera = build_camera_contract(_scene_report())
        track = {
            "id": "person_single_sample",
            "class_name": "person",
            "continuity_confidence": 0.9,
            "detections": [
                {"frame": 112, "bbox": [122, 100, 142, 200], "confidence": 0.9},
        ...
        self.assertIsNotNone(prediction)
        self.assertEqual("continuous", prediction["lifecycle"])
        self.assertEqual(0.0, prediction["speed_meters_per_second"])
