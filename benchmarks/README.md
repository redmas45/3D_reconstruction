...
```bash
python scripts/run_m0_benchmark.py --quick --label local_smoke
```

Full local sweep:

```bash
python scripts/run_m0_benchmark.py --label local
```
...
Commit the resulting `benchmarks/m0_colab_t4.json`. It is the input to the M1 render
