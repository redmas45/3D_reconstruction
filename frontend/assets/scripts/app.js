...
import { createReconstructionPlaybackCard } from "./dashboard/timeline-reconstruction-player.js";
...
const STAGE_LABELS = {
  ...
  building_scene: "Building 3D scene",
  rendering: "Rendering",
...
const PIPELINE_STEPS = Object.freeze([
  ...
  { stage: "building_scene", label: "Build validated 3D scene manifest", start: 0.58, end: 1 },
]);
...
/** @type {Map<string, {dispose: () => void}>} Live 3D dashboards, disposed on card removal. */
const reconDashboardsByJobId = new Map();
...
function createOutputCard(job) {
  // Browser-first jobs carry both the validated scene manifest and a complete playback
  // base. The timeline player synchronizes the transparent Three.js layer to the video.
  if (job.scene) return createSceneOutputCard(job);
...
/** @param {import("./api-client.js").ProcessingJob} job @returns {HTMLElement} */
function createSceneOutputCard(job) {
  const card = createElement("article", "output-card recon-output");
  try {
    const playback = createReconstructionPlaybackCard(
      job.scene,
      job.output_url || "",
      job.source_name,
    );
    reconDashboardsByJobId.set(job.id, playback);
    card.append(playback.element);
  } catch (error) {
    card.append(createElement("p", "recon-note", "The 3D scene could not be prepared for display."));
  }
  const actions = createElement("div", "output-actions");
  const download = createElement("a", "action-button", "Download evidence-safe timeline");
  download.setAttribute("title", "Downloads the continuous base timeline; the animated 3D layer is shown in the browser review.");
  download.setAttribute("href", job.download_url || "");
...
function disposeReconDashboard(jobId) {
  const dashboard = reconDashboardsByJobId.get(jobId);
  if (dashboard) {
    dashboard.dispose();
    reconDashboardsByJobId.delete(jobId);
  }
}
