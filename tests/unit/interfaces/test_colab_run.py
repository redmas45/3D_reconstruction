...
CONFIGURATION_PATH = PROJECT_ROOT / "config" / "reconstruction_config.json"


class TestEnvironmentFileParsing:
    def test_reads_the_three_azure_values(self):
        payload = (
            b"AZURE_OPENAI_API_KEY=abc123\n"
            b"AZURE_OPENAI_BASE_URL=https://example.openai.azure.com/\n"
            b"AZURE_OPENAI_CHAT_DEPLOYMENT=gpt-5.4-mini\n"
        )
        values = parse_environment_file(payload)
        assert set(values) == set(AZURE_SECRET_NAMES)
        assert values["AZURE_OPENAI_API_KEY"] == "abc123"

    def test_ignores_comments_and_blank_lines(self):
        payload = b"# a comment\n\nAZURE_OPENAI_API_KEY=abc\n"
        assert parse_environment_file(payload)["AZURE_OPENAI_API_KEY"] == "abc"

    def test_accepts_export_prefixed_assignments(self):
        payload = b"export AZURE_OPENAI_API_KEY=abc\n"
        assert parse_environment_file(payload)["AZURE_OPENAI_API_KEY"] == "abc"

    @pytest.mark.parametrize("quote", [b"'", b'"'])
    def test_strips_matching_quotes(self, quote):
        payload = b"AZURE_OPENAI_API_KEY=" + quote + b"abc" + quote + b"\n"
        assert parse_environment_file(payload)["AZURE_OPENAI_API_KEY"] == "abc"
